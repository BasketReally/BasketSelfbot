...
if os.name == "nt":
    ...
    os.system("title Basket selfbot")
if os.name == "posix":
    os.system("clear")

print(" ")
print(f"{printSpaces}Loading Bakset...")
print(" ")
...
try: 
    ...
    def update_config():
        ...
        if ("ping_detect" in configFile):
            configJson.pop("ping_detect")
            print(printSpaces+"Removing ping detect from config.")

        if ("ping" not in configJson["webhooks"]):
            configJson["webhooks"]["ping"] = ""
            print(printSpaces+"Adding ping webhook to config.")
        ...
        if ("detections" not in configFile):
            ...
            configJson["detections"]["ping"] = bool(True)
            print(printSpaces+"Adding ping detection to config.")
        ...
        if ("ignored_servers" not in configFile):
            ...
            configJson["ignored_servers"]["pings"] = []
            print(printSpaces+"Adding pings ignored servers to config.")
    ...
    if not os.path.isfile("data/icon.png"): open("data/icon.png", "wb").write(requests.get("https://raw.githubusercontent.com/BasketReally/BasketSelfbot/main/basket.png", allow_redirects=True).content)
...
{
    "enabled": true,
    "client_id": 1000887071013085244,
    "details": "Using Basket selfbot...",
    "state": "",
    "large_image_key": "basket",
    "large_image_text": "vk.com/basketr"
}
        """)
        f.close()

#notify
    if not os.path.isfile('themes/Basket.json'):
        f = open('themes/Basket.json', "w")
        f.write("""
{
    "embedtitle": "Basket selfbot v3.4",
    "embedcolour": "#3B79FF",
    "consolecolour": "#3B79FF",
    "embedfooter": "killmepls#4569",
    "embedfooterimage": "https://cdn.discordapp.com/attachments/961989807121240144/999289607789223987/ca1e89b50145520c298193fd154afce0.png",
...
Never gonna tell a lie and hurt...""")
    ...
    if not os.path.isfile("scripts/consolecommand-example.py"):
        ...
        f.write("""
@basket.command(name="consolecommand", description="console command test", usage="consoletest", aliases=["consoleCommand-consoletest"])
...
""")
    ...
    if not os.path.isfile('scripts/example.py'):
        ...
        f.write('''
@basket.command(name="example", description="Example custom script.", usage="example")
async def example(basket):
    exampleEmbed = discord.Embed(
        ...
        ** **
        Baskeet scripts are all created in python using discord.py so you can use any feature from discord.py.
        """,
    ...
    **\_\_deletetimeout\_\_** : Config delete timeout (seconds).
    ...
    await basket.send("Hello World!", embed=exampleEmbed)
    ''')
        f.close()
    ...
    __pingwebhook__ = CONFIG["webhooks"]["ping"]
    ...
    __pingdetect__ = CONFIG["detections"]["ping"]
    ...
    def close_bot():
        os.system("taskkill /IM basket.exe")
    def is_windows():
        return os.name == "nt"
    def is_linux():
    ...
    class Config():
        ...
        def changePrefix(newPrefix):
            ...
            basket.command_prefix = newPrefix
            cfg = Config.getConfig()
    ...
    if requests.get("https://discord.com/api/users/@me/settings", headers={"Authorization": __token__}).status_code == 200:
        ...
        status = "online"
    basket = commands.Bot(command_prefix=__prefix__, self_bot=True, status=discord.Status.try_value(status))
    basket.remove_command('help')
    basket.launch_time = datetime.utcnow()
    ...
    async def on_connect():
        ...
        if is_windows():
            os.system("cls")
            os.system(f"title Basket's selfbot [{version}] [{basket.user}]")
        ...
        threading.Thread(target=constant_input2, args=(basket,)).start()
                     
        if consoleMode.lower() == "ilovebasket":
            print(rf""" 
            {lred}
            {lred}    /\___/\
            {lred}   /       \
            {lred}  l  u   u  l    
            {lred}--l----*----l--                 {lred} ___           __       __    
            {lred}   \   w   /                    {lred} / _ )___ ____ / /_____ / /____        {lred}Discord: {white}killmepls#4569
            {white}     ======                    {white} / _  / _ `(_-</  '_/ -_) __(_-<        {lred}VK: {white}/basketr
            {white}   /       \ __                {white}/____/\_,_/___/_/\_\\__/\__/___/
            {white}   l        l\ \                                                         {lred}Basket selfbot {lgreen}{version}{lgreen}
            {white}   l        l/ /                       
            {white}   l  l l   l /              
            {white}   \ ml lm /_/ {reset}
            """)
        ...
        if "beta" in version.lower():
            print_important("You're currently using a beta build of Basket.")
            print_important("If you notice any bugs please report them to the developer.")
            print(" ")
        elif "dev" in version.lower():
            print_important("You're currently using a developer build of Basket.")
            print_important("If you notice any bugs please report them to the developer.")
            print(" ")                
        ...
        print_info(f"Basket selfbot can now be used with {basket.command_prefix} prefix.")
        send_notification("Basket selfbot", "Successfully connected!", 10)  
        global __basketloaded__
        __basketloaded__ = True
    ...
    async def on_message_delete(message):
        if __basketloaded__:
            ...
            if Config.getConfig()["detections"]["ping"]:
                if basket.user.mentioned_in(message):
                    if message.guild.id not in __ignoredservers__["pings"]:
                        print_detect(" Ping")
                        ...
                        if Config.getConfig()["sounds"]:   
                            if str(sounddevice.query_devices()) != "":          
                                ...
                                pygame.mixer.music.play(1)                        
                        send_notification(" Ping", f"You were pinged in {message.guild} by {message.author}.", 10)
                        if __pingwebhook__ != "":
                            webhook = DiscordWebhook(url=__pingwebhook__)
                            embed = DiscordEmbed(title='Ping', color=__embedcolourraw__[1:], description=f"`{message.author}` pinged you in `{message.channel}` (`{message.guild}`)")
    ...
    async def on_message(message):
        if __basketloaded__:
            ...
            if message.author.id != basket.user.id:
...
# {__embedfooter__}```""", delete_after=__deletetimeout__)
    ...
    @basket.command(name="customcommands", description="Display all custom commands.", usage="customcommands", aliases=["ccmds", "usercommands", "ucmds"])
...
{__embedfooter__}```""", delete_after=__deletetimeout__)
    ...
    @basket.command(name="allcmds", description="Print a list of all the commands.", usage="allcmds", aliases=["features"])
    async def allcmds(ctx):
        await ctx.message.delete()
        content = ""
        totalCommands = len(basket.commands)
        for command in basket.commands:
        ...
        if __embedmode__:
            ...
            embed.add_field(name="Commands", value="https://github.com/BasketReally/BasketSelfbot/blob/main/features.txt")
            embed.set_author(name="All Basket SelfBot Commands")
            embed.set_thumbnail(url=__embedimage__)
            embed.set_image(url=__embedlargeimage__)
            embed.set_footer(text=__embedfooter__, icon_url=__embedfooterimage__)
            await ctx.send(embed=embed, delete_after=__deletetimeout__)
        else:
            await ctx.send("https://github.com/BasketReally/BasketSelfbot/blob/main/features.txt")

    @basket.command(name="search", description="Search for commands.", usage="search [term]", aliases=["find", "searchcommand"])
...
```""", delete_after=__deletetimeout__)
            ...
            text2 = ""
            searchedItems = 0
            for cmd in basket.commands:
                if command in cmd.name or command in cmd.description or command in cmd.aliases:
                    searchedItems += 1
                    text += f"`{basket.command_prefix}`**{cmd.usage}** » {cmd.description}\n"
                    text2 += f"{basket.command_prefix}{cmd.usage} » {cmd.description}\n"
...
# {__embedfooter__}```""", delete_after=__deletetimeout__)
    ...
    @basket.command(name="help", description="The help command.", usage="help (command)", aliases=["cmds", "commands"])
    async def help(ctx, *, command = None):
        totalcmds = len(basket.commands)-len(scriptsList)
        if command is None:
            if __embedmode__:
                embed = discord.Embed(title=f"{__embedemoji__} **{__embedtitle__}** {__embedemoji__}", color=__embedcolour__, description=f"""
Arguments in `[]` are required, arguments in `()` are optional.

`{basket.command_prefix}`**text (page 1/2)** » Text commands.
`{basket.command_prefix}`**fun (page 1)** » Fun commands.
`{basket.command_prefix}`**image (page 1)** » Image commands.
`{basket.command_prefix}`**moderation (page 1)** » Moderation commands.
`{basket.command_prefix}`**info (page 1)** » Info commands.
`{basket.command_prefix}`**user (page 1)** » User commands.
`{basket.command_prefix}`**selfbot (page 1)** » Selfbot commands.
`{basket.command_prefix}`**webhook (page 1)** » Webhook commands.
`{basket.command_prefix}`**abuse (page 1)** » Abuse commands.
`{basket.command_prefix}`**themes (page 1)** » Theme commands.
`{basket.command_prefix}`**giveaway (page 1)** » Giveaway commands.
`{basket.command_prefix}`**nsfw (page 1)** » NSFW commands.
`{basket.command_prefix}`**proxy (page 1)** » Proxy commands.
`{basket.command_prefix}`**tools (page 1)** » Discord and other tools.
`{basket.command_prefix}`**customcommands** » Your custom commands.
`{basket.command_prefix}`**customscripts** » Your scripts.

`{basket.command_prefix}`**search [term]** » Search for a command.
`{basket.command_prefix}`**help (command)** » Help for a specific command.
...
# {__embedfooter__}```""", delete_after=__deletetimeout__)
    ...
    @basket.command(name="themes", description="Theme related commands.", usage="themes")
    async def themes(ctx):
        ...
        if __embedmode__:
            ...
            embed.add_field(name="Commands", value=f"`{basket.command_prefix}`**newtheme [name]** » Create a new theme with the given name.\n`{basket.command_prefix}`**deltheme [name]** » Delete the named theme.\n`{basket.command_prefix}`**theme [theme]** » Change your current theme.\n`{basket.command_prefix}`**ctheme** » Community themes.", inline=False)
...
[ Commands ]
{basket.command_prefix}newtheme [name] » Create a new theme with the given name.
{basket.command_prefix}deltheme [name] » Delete the named theme.
{basket.command_prefix}theme [theme] » Change your current theme.
{basket.command_prefix}cthemes » Community themes.


# {__embedfooter__}```""", delete_after=__deletetimeout__)

    @basket.command(name="ctheme", description="Community themes.", usage="ctheme", aliases=["communitythemes", "cloudthemes", "cthemes"])
    async def ctheme(ctx, *, dl = None):
        if dl is None:
            url = "https://raw.githubusercontent.com/BasketReally/Community-Themes/main/themes.txt"
            themes = requests.get(url).text.split("\n")

            if __embedmode__:
                embed = discord.Embed(title=f"{__embedemoji__} **{__embedtitle__}** {__embedemoji__}", description=f"Community Themes, run `{basket.command_prefix}ctheme (theme name)` to download the theme.\n ", color=__embedcolour__)
...
Community Themes, run {basket.command_prefix}ctheme (theme name) to download the theme.
...
# {__embedfooter__}```""", delete_after=__deletetimeout__)
            ...
            request = requests.get("https://raw.githubusercontent.com/BasketReally/Community-Themes/main/themes.txt")
            themes = []
            for line in request.text.split("\n"):
                themes.append(line.replace("\r", ""))

            if dl in themes:
                url = f'https://raw.githubusercontent.com/BasketReally/Community-Themes/main/{dl}.json'
...
# {__embedfooter__}```""", delete_after=__deletetimeout__)
    ...
    @basket.command(name="text", description="Text related commands.", usage="text (page)")
    async def text(ctx, page:int = 1):
        if __embedmode__:
            if page == 1:
                embed = discord.Embed(title=f"{__embedemoji__} **{__embedtitle__}** {__embedemoji__}", color=__embedcolour__, description=f"""
`{basket.command_prefix}`**js [message]** » Send all your messages in a JavaScript code block.
`{basket.command_prefix}`**lua [message]** » Send all your messages in a Lua code block.
`{basket.command_prefix}`**php [message]** » Send all your messages in a PHP code block.
`{basket.command_prefix}`**html [message]** » Send all your messages in a HTML code block.
`{basket.command_prefix}`**css [message]** » Send all your messages in a CSS code block.
`{basket.command_prefix}`**yaml [message]** » Send all your messages in a YAML code block.
`{basket.command_prefix}`**json [message]** » Send all your messages in a JSON code block.
`{basket.command_prefix}`**cpp [message]** » Send all your messages in a C++ code block.
`{basket.command_prefix}`**cs [message]** » Send all your messages in a C# code block.
`{basket.command_prefix}`**java [message]** » Send all your messages in a Java code block.
`{basket.command_prefix}`**python [message]** » Send all your messages in a Python code block.
`{basket.command_prefix}`**secret [message]** » Send all your messages in a secret block.
`{basket.command_prefix}`**secretletters [message]** » Put all lettes from your message into separate secret blocks
`{basket.command_prefix}`**regional [message]** » Replace all letters with emoji.
`{basket.command_prefix}`**bold [message]** » Send all your messages in bold.
`{basket.command_prefix}`**italic [message]** » Send all your messages in italics.
            """)
                ...
                embed = discord.Embed(title=f"{__embedemoji__} **{__embedtitle__}** {__embedemoji__}", color=__embedcolour__, description=f"""
`{basket.command_prefix}`**rembed (delay) [title]** » Kill Discord's API with a sexy rainbow embedded message.
`{basket.command_prefix}`**cembed [title] [description] [colour]** » Create a custom embedded message.
`{basket.command_prefix}`**embed [title]** » Create an embedded message.
`{basket.command_prefix}`**suggest [suggestion]** » Suggest something.
`{basket.command_prefix}`**privatemsg [message]** » Send an encrypted message.
`{basket.command_prefix}`**privatemsgdecode [message]** » Decode an encrypted message.
`{basket.command_prefix}`**blank** » Send a blank message
`{basket.command_prefix}`**length [string]** » Get the length of a string.
`{basket.command_prefix}`**chatbypass [text]** » Bypass chat language restrictions.
`{basket.command_prefix}`**shrug** » Shrug your arms.
`{basket.command_prefix}`**tableflip** » Flip the table.
`{basket.command_prefix}`**unflip** » Put the table back.
`{basket.command_prefix}`**lmgtfy [search]** » Let me Google that for you.
`{basket.command_prefix}`**typing [start/stop]** » Start or stop typing.
`{basket.command_prefix}`**aesthetic [text]** » Send your text s p a c e d out.
`{basket.command_prefix}`**lowercase [msg]** » Send your message in lowercase.
`{basket.command_prefix}`**uppercase [msg]** » Send your message in uppercase.
`{basket.command_prefix}`**sentencecase [msg]** » Send your messages in sentence case.
`{basket.command_prefix}`**ascii [text]** » Send your message in ascii.
`{basket.command_prefix}`**zalgo [text]** » Unleash the zalgo into your message.  
`{basket.command_prefix}`**leet [text]** » Turn your text into 1337 text.
`{basket.command_prefix}`**fakeedited [message]** » "Edit" a message.
`{basket.command_prefix}`**brainfuck [text]** » Generate brainfuck code from text.
`{basket.command_prefix}`**executebrainfuck [code]** » Execute brainfuck code.
            """)
...
# {__embedfooter__}```""", delete_after=__deletetimeout__)
    ...
    @basket.command(name="fun", description="Fun related commands.", usage="fun")
    async def fun(ctx, page:int = 1):
        if __embedmode__:
            if page == 1:
                embed = discord.Embed(title=f"{__embedemoji__} **{__embedtitle__}** {__embedemoji__}", color=__embedcolour__, description=f"""
`{basket.command_prefix}`**slots** » Play the slot machine.
`{basket.command_prefix}`**yomomma** » Random yo momma joke.
`{basket.command_prefix}`**socialcredit [@user]** » A users social credit score. 
`{basket.command_prefix}`**roast [@user]** » Roast a user.
`{basket.command_prefix}`**howgay [@user]** » How gay a user is.
`{basket.command_prefix}`**howskid [@user]** » Check the percentage of a skid.
`{basket.command_prefix}`**halal [@user]** » Checks if a user is halal or haram.
`{basket.command_prefix}`**iq [@user]** » Check how smart a user is.
`{basket.command_prefix}`**pp [@user]** » The length of a user's penis.
`{basket.command_prefix}`**rainbowrole [@role]** » Kill Discord's API with a sexy rainbow role.
`{basket.command_prefix}`**coinflip** » Flip a coin.
`{basket.command_prefix}`**dice** » Roll a dice.
`{basket.command_prefix}`**8ball [question]** » Ask the magic eight ball a question.
`{basket.command_prefix}`**choice [choice1] [choice2]** » Pick a random choice.
`{basket.command_prefix}`**range [number1] [number2]** » Choose a random number between two.
`{basket.command_prefix}`**dox [@user]** » Dox the mentioned user.
`{basket.command_prefix}`**fakenitro [url]** » Hide a link in a nitro URL.
`{basket.command_prefix}`**purgehack** » Purge without permissions.
`{basket.command_prefix}`**dadjoke** » A random dad joke.
`{basket.command_prefix}`**randommessage** » A random message.
`{basket.command_prefix}`**randomquestion** » A random question.
`{basket.command_prefix}`**rickroll** » Send never gonna give you up lyrics one by one.
`{basket.command_prefix}`**stoprickroll** » Stop sending rick astley lyrics.
`{basket.command_prefix}`**countdown [number]** » Count down from a number.
`{basket.command_prefix}`**countup [number]** » Count up from a number.
`{basket.command_prefix}`**pytoexe [path]** » Convert a PY file to an executable.
`{basket.command_prefix}`**skin [name]** » Gets the skin of a MC user.
            """)
...
# {__embedfooter__}```""", delete_after=__deletetimeout__)
    ...
    @basket.command(name="image", description="Image related commands.", usage="image", aliases=["img"])
    async def image(ctx, page:int = 1):
        if __embedmode__:
            if page == 1:
                embed = discord.Embed(title=f"{__embedemoji__} **{__embedtitle__}** {__embedemoji__}", color=__embedcolour__, description=f"""
`{basket.command_prefix}`**meme** » A random meme.
`{basket.command_prefix}`**cat** » A random cat image.
`{basket.command_prefix}`**dog** » A random dog image.
`{basket.command_prefix}`**shiba** » A random shiba image.
`{basket.command_prefix}`**fox** » A random fox image. (Thanks Imf44 <3)
`{basket.command_prefix}`**avatar [@user]** » Get the mentioned user's avatar.
`{basket.command_prefix}`**servericon** » Get the server's icon. 
`{basket.command_prefix}`**achievement ["text"] (icon)** » Create a fake minecraft achievement image.
`{basket.command_prefix}`**challenge ["text"] (icon)** » Create a fake minecraft challenge image.
`{basket.command_prefix}`**captcha [text]** » Create a fake reCaptcha.
`{basket.command_prefix}`**amiajoke [@user]** » Make a user a joke.
`{basket.command_prefix}`**didyoumean ["text 1"] ["text 2"]** » Create a google did you mean image.
`{basket.command_prefix}`**drake ["text 1"] ["text 2"]** » Create a drake meme image.
`{basket.command_prefix}`**facts [text]** » Create a facts meme image.
`{basket.command_prefix}`**jokeoverhead [image url]** » Create a joke over head image.
`{basket.command_prefix}`**pornhub ["text 1"] ["text 2"]** » Create a pornhub logo image.
`{basket.command_prefix}`**salty [@user]** » Make someone salty.
`{basket.command_prefix}`**ship [@user 1] [@user 2]** » Ship two people.
`{basket.command_prefix}`**trash [@user]** » Put someone in the trash.
`{basket.command_prefix}`**what [image url]** » Make a what meme.
...
# {__embedfooter__}```""", delete_after=__deletetimeout__)
    ...
    @basket.command(name="nsfw", description="NSFW related commands.", usage="nsfw")
    async def nsfw(ctx, page:int = 1):
        if __embedmode__:
            if page == 1:
                embed = discord.Embed(title=f"{__embedemoji__} **{__embedtitle__}** {__embedemoji__}", color=__embedcolour__, description=f"""
`{basket.command_prefix}`**boobs** » Pictures or videos of boobs.
`{basket.command_prefix}`**ass** » Pictures or videos of ass.
`{basket.command_prefix}`**pussy** » Pictures or videos of pussy.
`{basket.command_prefix}`**porngif** » Porn gifs.
`{basket.command_prefix}`**hentai** » Pictures or videos of hentai.
            """)
...
# {__embedfooter__}```""", delete_after=__deletetimeout__)
    ...
    @basket.command(name="moderation", description="Moderation related commands.", usage="moderation")
    async def moderation(ctx, page:int = 1):
        if __embedmode__:
            if page == 1:
                embed = discord.Embed(title=f"{__embedemoji__} **{__embedtitle__}** {__embedemoji__}", color=__embedcolour__, description=f"""
`{basket.command_prefix}`**ban [@user]** » Ban the mentioned user.
`{basket.command_prefix}`**unban [id]** » Unban the mentioned id.
`{basket.command_prefix}`**banlist** » See the server's ban list.
`{basket.command_prefix}`**kick [@user]** » Kick the mentioned user.
`{basket.command_prefix}`**mute [@user]** » Mute the menitioned user.
`{basket.command_prefix}`**unmute [@user]** » Unmute the mentioned user.
`{basket.command_prefix}`**newrole [name]** » Create a new role.
`{basket.command_prefix}`**delrole [@role]** » Delete the mentioned role.
`{basket.command_prefix}`**purge [amount]** » Purge X amount of messages.
`{basket.command_prefix}`**lock** » Lock the command channel.
`{basket.command_prefix}`**unlock** » Unlock the command channel.
`{basket.command_prefix}`**lockdown** » Lock the entire server.
`{basket.command_prefix}`**unlockdown** » Unlock the entire server.
`{basket.command_prefix}`**spacechannel [channel name]** » Create a channel with spaces.
            """)
...
# {__embedfooter__}```""", delete_after=__deletetimeout__)
    ...
    @basket.command(name="info", description="Info related commands.", usage="info")
    async def info(ctx, page:int = 1):
        if __embedmode__:
            if page == 1:
                embed = discord.Embed(title=f"{__embedemoji__} **{__embedtitle__}** {__embedemoji__}", color=__embedcolour__, description=f"""
`{basket.command_prefix}`**userinfo [@user]** » Information about the mentioned user.
`{basket.command_prefix}`**serverinfo** » Information about the command server.
`{basket.command_prefix}`**watchdogstats** » Get stats about Hypixel's Anticheat, Watchdog.
`{basket.command_prefix}`**getmessage [message id]** » Get a message by ID.
`{basket.command_prefix}`**geoip [ip]** » Get information from an IP address.
`{basket.command_prefix}`**ping [ip/domain]** » Ping a domain or ip address. 
`{basket.command_prefix}`**crypto [currency]** » Get the current data on a cryptocurrency.
            """)
...
# {__embedfooter__}```""", delete_after=__deletetimeout__)
    ...
    @basket.command(name="user", description="User related commands.", usage="user")
    async def user(ctx, page:int = 1):
        if __embedmode__:
            if page == 1:
                embed = discord.Embed(title=f"{__embedemoji__} **{__embedtitle__}** {__embedemoji__}", color=__embedcolour__, description=f"""
`{basket.command_prefix}`**purgeself [amount]** » Purge your messages.
`{basket.command_prefix}`**statuscycle** » Start a custom status cycle.
`{basket.command_prefix}`**statuscycletext [text]** » Set the text used in status cycle.
`{basket.command_prefix}`**clearstatus** » Clear your status.
`{basket.command_prefix}`**nickname [text]** » Set your nickname to anything.
`{basket.command_prefix}`**clearnickname** » Clear your nickname.
`{basket.command_prefix}`**ppin [message id]** » Add a message to your personal pins.
`{basket.command_prefix}`**ppins** » List all your pinned messages.
`{basket.command_prefix}`**ppindel [pin id]** » Delete a pin from your personal pins.
`{basket.command_prefix}`**backupfriends** » Backup all your friend's user IDs to a file.
`{basket.command_prefix}`**backupservers** » Backup all your servers and try to create invites for each one.
`{basket.command_prefix}`**changehypesquad [bravery/brilliance/balance]** » Change your hypesquad house.
            """)
...
# {__embedfooter__}```""", delete_after=__deletetimeout__)
    ...
    @basket.command(name="selfbot", description="Selfbot related commands.", usage="selfbot")
    async def selfbot(ctx, page:int = 1):
        if __embedmode__:
            if page == 1:
                embed = discord.Embed(title=f"{__embedemoji__} **{__embedtitle__}** {__embedemoji__}", color=__embedcolour__, description=f"""
`{basket.command_prefix}`**afkmode** » Toggle afk mode.
`{basket.command_prefix}`**settings** » The bot's settings.
`{basket.command_prefix}`**restart** » Restart basket selfbot.
`{basket.command_prefix}`**prefix [prefix]** » Set the command prefix.
`{basket.command_prefix}`**dumpchat [amount] (channel id) (oldest first, true/false)** » Get the chat's history.
`{basket.command_prefix}`**invite** » Get basket's Discord server invite link. #notift
`{basket.command_prefix}`**addccmd [name] [response]** » Add a custom command.
`{basket.command_prefix}`**delccmd [name]** » Delete a custom command.
`{basket.command_prefix}`**detections** » A list of all detections.
`{basket.command_prefix}`**snipers** » A list of all snipers.
`{basket.command_prefix}`**enablesniper [type]** » Enable a sniper.
`{basket.command_prefix}`**disablesniper [type]** » Disable a sniper.
`{basket.command_prefix}`**enabledetect [type]** » Enable a detection.
`{basket.command_prefix}`**disabledetect [type]** » Disable a detection.
`{basket.command_prefix}`**riskmode** » Disable and enable risk mode.
`{basket.command_prefix}`**sounds** » Toggle notification sounds.
`{basket.command_prefix}`**notifications** » Toggle notification.
            """)
...
{basket.command_prefix}prefix [prefix] » Set the command prefix.
{basket.command_prefix}dumpchat [amount] (channel id) (oldest first, true/false) » Get the chat's history.
{basket.command_prefix}invite » Get basket's Discord server invite link.
{basket.command_prefix}addccmd [name] [response] » Add a custom command.
{basket.command_prefix}delccmd [name] » Delete a custom command.
{basket.command_prefix}detections » A list of all detections.
{basket.command_prefix}snipers » A list of all snipers.
...
# {__embedfooter__}```""", delete_after=__deletetimeout__)
    ...
    @basket.command(name="webhook", description="Webhook related commands.", usage="webhook")
    async def webhook(ctx, page:int = 1):
        if __embedmode__:
            if page == 1:
                embed = discord.Embed(title=f"{__embedemoji__} **{__embedtitle__}** {__embedemoji__}", color=__embedcolour__, description=f"""
`{basket.command_prefix}`**delwebhook [id]** » Delete a webhook from the ID.
`{basket.command_prefix}`**newwebhook [name]** » Create a webhook in the command channel.
`{basket.command_prefix}`**spamwebhook [amount] [url] (message)** » Spam the shit out of a webhook.
`{basket.command_prefix}`**webhooksetup** » Creates a new server with webhooks.
`{basket.command_prefix}`**webhookinfo [id]** » Information about the webhook.
            """)
...
# {__embedfooter__}```""", delete_after=__deletetimeout__)
    ...
    @basket.command(name="abuse", description="Abuse related commands.", usage="abuse")
    async def abuse(ctx, page:int = 1):
        if __riskmode__:
            if __embedmode__:
                if page == 1:
                    embed = discord.Embed(title=f"{__embedemoji__} **{__embedtitle__}** {__embedemoji__}", color=__embedcolour__, description=f"""
`{basket.command_prefix}`**spam [amount] [delay] [message]** » Spam X amount of times.
`{basket.command_prefix}`**stopspam** » Stop spamming messages.
`{basket.command_prefix}`**dmspam [amount] [delay] [@user] [message]** » Spam DM messages X amount of times.
`{basket.command_prefix}`**channelspam [amount] [delay] [message]** » Spam a message X amount of times in every channel.
`{basket.command_prefix}`**threadspam [delay] [amount] [addusers | true/false] [name] [startmessage]** » Spam create threads with a starting message.
`{basket.command_prefix}`**ttsspam [amount] [delay] [message]** » Spam TTS messages X amount of times.
`{basket.command_prefix}`**reactspam [emoji] [messages]** » Spam reactions on X amount of messages.
`{basket.command_prefix}`**massghostping [delay] [@user]** » Ghost Ping the user in every channel.
`{basket.command_prefix}`**ghostping [@user]** » Ping a user then delete the message.
`{basket.command_prefix}`**massping (amount of messages) (send delay)** » Ping a mass amount of people in the command server.
`{basket.command_prefix}`**massnick [nickname]** » Change the nickname of all members in the command server.
`{basket.command_prefix}`**massdm [delay] [amount] [message]** » Send a DM message to everyone in the server.
`{basket.command_prefix}`**nukeserver** » Delete all roles and channels in the command server.
`{basket.command_prefix}`**destroyserver** » Completely destroy the command server.
`{basket.command_prefix}`**deletechannels** » Delete all of the command server's channels.
`{basket.command_prefix}`**deleteroles** » Delete all of the command server's roles.
`{basket.command_prefix}`**spamchannels [amount] (name)** » Spam create channels with a desired name. (Thanks Port <3)
`{basket.command_prefix}`**spamroles [amount] (name)** » Spam create roles with a desired name.
`{basket.command_prefix}`**raidjoin [delay] [invite]** » Make all your account tokens join a server.
`{basket.command_prefix}`**tokenraid [amount] [channel id] (message)** » Raid a server with all your account tokens.
`{basket.command_prefix}`**massban** » Ban all the members in the command server.
`{basket.command_prefix}`**masskick** » Kick all the members in the command server.
                """)
...
{basket.command_prefix}stopspam » Stop spamming messages.
{basket.command_prefix}dmspam [amount] [delay] [@user] [message] » Spam DM messages X amount of times.
{basket.command_prefix}channelspam [amount] [delay] [message] » Spam X amount of times in all channels.
{basket.command_prefix}threadspam [delay] [amount] [name] [startmessage] » Spam create threads with a starting message.
{basket.command_prefix}ttsspam [amount] [delay] [message] » Spam TTS messages X amount of times.
{basket.command_prefix}reactspam [emoji] [messages] » Spam reactions on X amount of messages.
{basket.command_prefix}massghostping [delay] [@user] » Ghost Ping the user in every channel.
{basket.command_prefix}ghostping [@user] » Ping a user then delete the message.
{basket.command_prefix}massping » Ping a mass amount of people in the command server.
{basket.command_prefix}massnick [nickname] » Change the nickname of all members in the command server.
{basket.command_prefix}massdm [delay] [amount] [message] » Send a DM message to everyone in the server.
{basket.command_prefix}nukeserver » Delete all roles and channels in the command server.
{basket.command_prefix}destroyserver » Completely destroy the command server.
...
# {__embedfooter__}```""", delete_after=__deletetimeout__)                                          
    ...
    @basket.command(name="tools", description="Discord and other tools.", usage="tools", aliases=["utilities", "utils"])
    async def tools(ctx, page:int = 1):
        if __embedmode__:
            if page == 1:
                embed = discord.Embed(title=f"{__embedemoji__} **{__embedtitle__}** {__embedemoji__}", color=__embedcolour__, description=f"""
`{basket.command_prefix}`**tokeninfo [token]** » Information about a token. 
`{basket.command_prefix}`**nuketoken [token]** » Nuke a token.
`{basket.command_prefix}`**checktoken [token]** » Checks if a token is working.
`{basket.command_prefix}`**checktokens** » Check your tokens. 
`{basket.command_prefix}`**nitrogen** » Generate a nitro code.
`{basket.command_prefix}`**tokengen** » Generate a discord user token.
`{basket.command_prefix}`**identitygen** » Generate a fake identity.
`{basket.command_prefix}`**passwordgen [length]** » Generate a secure password.
`{basket.command_prefix}`**ccgen** » Generate a fake Credit card.
`{basket.command_prefix}`**eval** » very scary and haram.
            """)
...
# {__embedfooter__}```""", delete_after=__deletetimeout__)
    ...
    @basket.command(name="giveaway", description="Giveaway related commands.", usage="giveaway")
...
`{basket.command_prefix}`**gstart [duration] [winners] [prize]** » Start a giveaway in the same channel
`{basket.command_prefix}`**gend [message id]** » End a giveaway
`{basket.command_prefix}`**greroll [message id]** » Re-roll a giveaway
            """)
...
# {__embedfooter__}```""", delete_after=__deletetimeout__)
    ...
    @basket.command(name="proxy", description="Proxy related commands.", usage="proxy")
    async def proxy(ctx, page:int=1):
        if __embedmode__:
            if page == 1:
                embed = discord.Embed(title=f"{__embedemoji__} **{__embedtitle__}** {__embedemoji__}",
                                    color=__embedcolour__, description=f"""
`{basket.command_prefix}`**proxies http** » Scrape HTTP proxies.
`{basket.command_prefix}`**proxies https** » Scrape HTTPS proxies.
`{basket.command_prefix}`**proxies socks4** » Scrape SOCKS4 proxies.
`{basket.command_prefix}`**proxies socks5** » Scrape SOCKS5 proxies.
`{basket.command_prefix}`**proxies all** » Scrape HTTP, HTTPS, SOCKS4 AND SOCKS5 proxies.
            """)
...
# {__embedfooter__}```""", delete_after=__deletetimeout__)
    ...
    @basket.command(name="hiddenchannels", description="Sends a list of all the channels you cant see.", usage="hiddenchannels (guild id)")
    async def hiddenchannels(ctx, guild=None):
        if guild == None:
            guild = ctx.guild
        else:
            guild = await basket.fetch_guild(int(guild))
        hiddenChannels = []
...
```""", delete_after=__deletetimeout__) 
    ...
    @basket.command(name="clearconsole", description="Clear your console.", usage="clearconsole", aliases=["resetconsole", "consoleclear", "consolereset", "consoleCommand-clearconsole", "consoleCommand-clear"])
    async def clearconsole(ctx):
        width = os.get_terminal_size().columns

        if is_windows():
            os.system("cls")
            os.system(f"title basket [{version}] [{basket.user}]")
        if is_linux():
            os.system("clear")
        if consoleMode.lower() == "new":
            ...
            {white}     ======           {white} / _  / _ `(_-</  '_/ -_) __(_-<        {lred}VK: {white}/basketr
            {white}   /       \ __       {white}/____/\_,_/___/_/\_\\__/\__/___/
            {white}   l        l\ \                                             {lred}Basket selfbot {lgreen}v2.3.9{green}
            {white}   l        l/ /                       
            {white}   l  l l   l /              
            {white}   \ ml lm /_/ 
            """)
        ...
        if "beta" in version.lower():
            print_important("You're currently using a beta build of Basket selfbot.")
            print_important("If you notice any bugs please report them to the developer.")
            print(" ")
        elif "dev" in version.lower():
            print_important("You're currently using a developer build of selfbot.")
            print_important("If you notice any bugs please report them to the developer.")
            print(" ")                      

    @basket.command(name="blocksend", description="Send a message to a blocked user.", usage="blocksend [user id] [messages]", aliases=["sendblocked", "sendtoblocked"])
    async def blocksend(ctx, userid:int, *, message):
        user = await basket.fetch_user(userid)
        await user.unblock()
    ...
    @basket.command(name="riskmode", description="Disable and enable risk mode", usage="riskmode")
    ...
    @basket.command(name="embedmode", description="Toggle embed mode.", usage="embedmode")
    ...
    @basket.command(name="textmode", description="Toggle text mode.", usage="textmode")
    ...
    @basket.command(name="readall", description="Mark every message as read.", usage="readall")
    async def readall(ctx):
        index = 0
        index2 = 0
        DiscumClient = discum.Client(token=__token__, log=False, user_agent=get_random_user_agent())
        for guild in basket.guilds:
            messages2 = []
    ...
    @basket.command(name="specs", description="Your computers specifications.", usage="specs", aliases=["computerspecs", "pcspecs", "specifications", "systeminfo"])
    async def specs(ctx):
...
```""", delete_after=__deletetimeout__)
    ...
    @basket.command(name="crypto", description="Get the current data on a cryptocurrency.", usage="crypto [currency]", aliases=["cryptodata", "cryptostats"])
...
```""")
    ...
    @basket.command(name="proxies", description="Scrape an type of proxy.", usage="proxies [http, https, socks4, socks5, all]", aliases=["proxygen", "genproxies", "proxyscrape"])
    ...
    @basket.command(name="cloneemoji", description="Clone an emoji to the command server.", usage="cloneemoji [emoji]", aliases=["stealemoji", "emojisteal", "copyemoji"])
    ...
    @basket.command(name="detections", description="A list of all detections.", usage="detections", aliases=["triggers"])
    ...
    @basket.command(name="snipers", description="A list of all snipers.", usage="snipers")
    ...
    @basket.command(name="enabledetect", description="Enable a detection.", usage="enabledetect [type]", aliases=["enabledetection", "enabledetections"])
    ...
    @basket.command(name="disabledetect", description="Disable a detection.", usage="disabledetect [type]", aliases=["disabledetection", "disabledetections"])
    ...
    @basket.command(name="enablesniper", description="Enable a sniper.", usage="enablesniper [type]", aliases=["enablesnipers"])
    ...
    @basket.command(name="disablesniper", description="Disable a sniper.", usage="disablesniper [type]", aliases=["disablesnipers"])
    ...
    @basket.command(name="addccmd", description="Add a custom command.", usage="addccmd [name] [response]", aliases=["addcustomcommand", "addusercommand"])
    async def addccmd(ctx, name, *, response):
        ...
        ccmd = json.load(open("customcommands.json"))
        await ctx.send(f"Added `{basket.command_prefix}{name}` to your custom commands.", delete_after=__deletetimeout__)

    @basket.command(name="delccmd", description="Remove a custom command.", usage="delccmd [name]", aliases=["deletecustomcommand", "delcustomcommand", "removecustomcommand", "removeccmd", "deleteccmd"])
    async def delccmd(ctx, name):
        ...
        json.dump(customCommands, open("customcommands.json", "w"), indent=4, sort_keys=False)
        ccmd = json.load(open("customcommands.json"))
        await ctx.send(f"Removed `{basket.command_prefix}{name}` from your custom commands", delete_after=__deletetimeout__)            

    @basket.command(name="boobs", description="Pictures or videos of boobs.", usage=f"boobs", aliases=["tits", "tit", "milkers", "titties", "boob", "melons"])
    async def boobs(ctx):
        type = "boobs"
        image = get_nsfw(type)
    ...
    @basket.command(name="ass", description="Pictures or videos of ass.", usage=f"ass", aliases=["badonkadonk"])
    async def ass(ctx):
        type = "ass"
        image = get_nsfw(type)
        if image.endswith("png") or image.endswith("jpeg") or image.endswith("jpg") or image.endswith("gif"):
    ...
    @basket.command(name="pussy", description="Pictures or videos of pussy.", usage=f"pussy")
    async def pussy(ctx):
        type = "pussy"
        image = get_nsfw(type)
        if image.endswith("png") or image.endswith("jpeg") or image.endswith("jpg") or image.endswith("gif"):
    ...
    @basket.command(name="porngif", description="Porn gifs.", usage=f"porngif")
    ...
    @basket.command(name="neko", description="Pictures or videos of nsfw nekos.", usage=f"hentai", aliases=["nsfwneko"])
    ...
    @basket.command(name="discordtheme", description="Change default Discord theme.", usage="discordtheme [light/dark]")
    ...
    @basket.command(name="changehypesquad", description="Change your hypesquad house.", usage="changehypesquad [bravery/brilliance/balance]")
    ...
    @basket.command(name="backupfriends", description="Backup all your friend's user IDs to a file.", usage="backupfriends", aliases=["friendbackup"])
    ...
    @basket.command(name="backupservers", description="Backup all your servers and try to create invites for each one.", usage="backupservers", aliases=["backupguilds", "serverbackup", "guildbackup"])
    async def backupservers(ctx):
        ...
        for item in guilds:
            ...
            server = discord.utils.get(basket.guilds, id=int(guildid))
    ...
    @basket.command(name="richpresence", description="Enable or disable rich presence.", usage="richpresence [on/off]", aliases=["rpc"])
    ...
    @basket.command(name="spacechannel", description="Create a channel with spaces.", usage="spacechannel [channel name]")
...
```""")
    ...
    @basket.command(name="uwu", description="Translate your messages to uwu!", usage="uwu [message]")
    async def uwu__(ctx, *, message):
        uwued = uwuify.uwu(message)
        await ctx.send(uwued)
        
    @basket.command(name="uwuify", description="Automatically translate all your sent messages to uwu!", usage="uwuify")
    ...
    @basket.command(name="geoip", description="Get information from an IP address.", usage="geoip [ip]", aliases=["iplookup", "lookupip", "ipinfo", "iplocation"])
...
```""", delete_after=__deletetimeout__)      
    ...
    @basket.command(name="pytoexe", description="Convert a PY file to an executable.", usage="pytoexe [path]", aliases=["pythontoexe", "py2exe", "python2exe"])
...
```""")          
    ...
    @basket.command(name="statuscycle", description="Start a custom status cycle.", usage="statuscycle", aliases=["cyclestatus"])
    async def statuscycle(ctx):
        ...
        basket.loop.create_task(loopStatus(cycleStatusText))
    ...
    @basket.command(name="statuscycletext", description="Set the text used in status cycle.", usage="statuscycletext [text]", aliases=["cyclestatustext"])
    ...
    @basket.command(name="ghostping", description="Ping a user then delete the message.", usage="ghostping [@user]")
    async def ghostping(ctx, user: discord.User):
        pass

    @basket.command(name="getmessage", description="Get a message by ID.", usage="getmessage [message id]", aliases=["fetchmessage", "getmsg", "fetchmsg"])
...
```""", delete_after=__deletetimeout__)      
    ...
    @basket.command(name="watchdogstats", description="Get stats about Hypixel's Anticheat, Watchdog", usage="watchdogstats", aliases=["hypixelstats", "banstats", "acstats"])
...
```""", delete_after=__deletetimeout__)
    ...
    @basket.command(name="skin", description="Gets a MC user skin", usage="skin [MC user]", aliases=["minecaftskin", "mcskin"])
    ...
    @basket.command(name="ppin", description="Add a message to your personal pins.", usage="ppin [message id]", aliases=["personalpin", "addppin", "addpersonalpin"])
    ...
    @basket.command(name="ppins", description="List all your pinned messages.", usage="ppins", aliases=["personalpins", "listppins", "ppinlist"])
    async def ppins(ctx):
        data = json.load(open("data/personal-pins.json"))
    ...
    @basket.command(name="ppindel", description="Delete a pin from your personal pins.", usage="ppindel [pin id]", aliases=["ppindelete", "removeppin", "deleteppin", "personalpindelete", "deletepersonalpin", "removepersonalpin"])
    ...
    @basket.command(name="countdown", description="Count down from a number.", usage="countdown [number]")
    async def countdown(ctx, number: int):
        for count in range(number, 0, -1):
            await ctx.send(count)

    @basket.command(name="countup", description="Count up from a number.", usage="countup [number]")
    async def countup(ctx, number: int):
        for count in range(number):
            await ctx.send(count)

    @basket.command(name="massban", description="Ban all the members in the command server.", usage="massban")
...
# {__embedfooter__}```""", delete_after=__deletetimeout__)                              
    ...
    @basket.command(name="masskick", description="Kick all the members in the command server.", usage="masskick")
...
# {__embedfooter__}```""", delete_after=__deletetimeout__)                              
    ...
    @basket.command(name="raidjoin", description="Make all your account tokens join a server.", usage="raidjoin [delay] [invite]")
...
# {__embedfooter__}```""", delete_after=__deletetimeout__)                              
    ...
    @basket.command(name="tokenraid", description="Raid a server with all your account tokens.", usage="tokenraid [threads] [amount] [channel id] (message)")
...
# {__embedfooter__}```""", delete_after=__deletetimeout__)                          
    ...
    @basket.command(name="checktoken", description="Checks if a token is working.", usage="checktoken [token]", aliases=["tokencheck"])
    ...
    @basket.command(name="checktokens", description="Checks if your tokens are working.", usage="checktokens")
    ...
    @basket.command(name="wipetoken", description="Completely wipe a token.", aliases=["cleantoken"])
    ...
    @basket.command(name="nuketoken", description="Nuke a token.", usage="nuketoken [token]", aliases=["tokennuke"])
    ...
    @basket.command(name="gstart", description="Start a giveaway in the same channel", usage="gstart [duration] [winners] [prize]", aliases=["giveawaystart", "startgiveaway"])
            ...
            if winners is not None:
                if prize is not None:
                    ...
                    if "🎉 **GIVEAWAY** 🎉" in msg.content:
                        ...
                        entries.remove(f"<@{basket.user.id}>")
                    ...
                    await ctx.send(
                        f"❌ **Incorrect Syntax**\nTry: `{basket.command_prefix}gstart 30m 1 Awesome T-Shirt`")
            else:
    ...
    @basket.command(name="gend", description="End a giveaway", usage="gend [message id]", aliases=["giveawayend", "endgiveaway"])
    async def gend(ctx, id: int = None):
        ...
        #print("Bot user ID: " + str(basket.user.id))

        if msgId == id and msgAuthorId == basket.user.id:
    ...
    @basket.command(name="greroll", description="Re-roll a giveaway", usage="greroll [message id]", aliases=["giveawayreroll", "rerollgiveaway"])
    async def greroll(ctx, id: int = None):
        ...
        if msg.author.id == basket.user.id:
            if "🎉 **GIVEAWAY** 🎉" in msg.content:
                #running = True
    ...
    @basket.command(name="typing", description="Start or stop typing.", usage="typing [start/stop]", aliases=["inftyping", "infintetyping"])
    ...
    @basket.command(name="sounds", description="Toggle notification sounds.", usage="sounds", aliases=["togglesounds", "soundstoggle"])
    ...
    @basket.command(name="notifications", description="Toggle notifications.", usage="notifications", aliases=["togglenotifications", "notificationstoggle", "togglenotifs"])
    ...
    @basket.command(name="ping", description="Ping a domain or ip address.", usage="ping [ip/domain]")
...
```""", delete_after=__deletetimeout__)
    ...
    @basket.command(name="cloneserver", description="Clone a server.", usage="cloneserver", aliases=["copyserver"])
    async def cloneserver(ctx):
        serverName = ctx.guild.name
        serverIcon = ctx.guild.icon

        newGuild = await basket.create_guild(serverName)
    ...
    @basket.command(name="webhooksetup", description="Create a new server with webhooks.", usage="webhooksetup", aliases=["setupwebhooks"])
    async def webhooksetup(ctx):
        global __nitrowebhook__, __privnotewebhook__, __giveawaywebhook__, __pingwebhook__, __friendsupdatewebhook__, __dmtypingwebhook__, __guildleavewebhook__, __selfbotwebhook__, __ticketswebhook__
        
        iconFile = open("data/icon.png", "rb")
        icon = bytes(iconFile.read())

        configFile = json.load(open("config.json"))
        guild = await basket.create_guild("Notifications", icon=icon)
        ...
        PingWebhookChannel = await category.create_text_channel("pings")
        ...
        nitroWebhook = await nitroWebhookChannel.create_webhook(name=" Nitro Sniper")
        privnoteWebhook = await privnoteWebhookChannel.create_webhook(name=" Privnote Sniper")
        giveawayWebhook = await giveawayWebhookChannel.create_webhook(name=" Giveaway Sniper")
        PingWebhook = await PingWebhookChannel.create_webhook(name=" Pings")
        ...
        __pingwebhook__ = PingWebhook.url
        ...
        configFile["webhooks"]["ping"] = __pingwebhook__
    ...
    @basket.command(name="spamwebhook", description="Spam the shit out of a webhook.", usage="spamwebhook [amount] [url] (message)")
    ...
    @basket.command(name="newwebhook", description="Create a webhook in the command channel.", usage="newwebhook [name]", aliases=["createwebhook"])
    ...
    @basket.command(name="delwebhook", description="Delete a webhook from the ID.", usage="delwebhook [id]", aliases=["deletewebhook", "removewebhook"])
    async def delwebhook(ctx, id: int):
        webhook = await basket.fetch_webhook(id)
    ...
    @basket.command(name="webhookinfo", description="Information about the webhook.", usage="webhookinfo [id]", aliases=["webhooklookup", "lookupwebhook"])
...
# {__embedfooter__}```""", delete_after=__deletetimeout__)
    ...
    @basket.command(name="dumpchat", description="Get the chat's history.", usage="dumpchat [amount] (channel id) (oldest first, true/false)", aliases=["savechat", "chathistory"])
    async def dumpchat(ctx, amount: int, channelId: int = None, oldestFirst: bool = False):
        if channelId is None:
            ...
            channel = basket.get_channel(channelId)
    ...
    @basket.command(name="newtheme", description="Create a new theme with the given name.", usage="newtheme [name]", aliases=["createtheme"])
...
# {__embedfooter__}```""", delete_after=__deletetimeout__)
    ...
    @basket.command(name="deltheme", description="Delete the named theme.", usage="deltheme [name]", aliases=["deletetheme", "removetheme"])
...
# {__embedfooter__}```""", delete_after=__deletetimeout__)
    ...
    @basket.command(name="theme", description="Change your current theme.", usage="theme [theme]", aliases=["settheme"])
...
# {__embedfooter__}```""", delete_after=__deletetimeout__)
    ...
    @basket.command(name="prefix", description="Set the command prefix.", usage="prefix [prefix]", aliases=["c"])
...
# {__embedfooter__}```""", delete_after=__deletetimeout__)
    ...
    @basket.command(name="restart", description="Restart basket selfbot.", usage="restart", aliases=["reboot", "reload", "consoleCommand-restart"])
    async def restart(ctx):
        print_info("Restarting basket...")
        try:
            await ctx.send("Restarting basket...")
        except:
            pass
        restart_bot()

    @basket.command(name="firstmessage", description="Get the first message in the command channel.", usage="firstmessage", aliases=["firstmsg"])
    ...
    @basket.command(name="brainfuck", description="Generate brainfuck code from text.", usage="brainfuck [text]", aliases=["bf"])
    async def brainfuck(ctx, *, text):
        result = brainfuckery.Brainfuckery().convert(text)
        await ctx.send(result)

    @basket.command(name="executebrainfuck", description="Execute brainfuck code.", usage="executebrainfuck [code]", aliases=["ebf"])
    async def executebrainfuck(ctx, *, code):
        result = brainfuckery.Brainfuckery().interpret(code)
        await ctx.send(result)

    @basket.command(name="shrug", description="Shrug your arms.", usage="shrug")
    async def shrug(ctx):
        await ctx.send(f"¯\_(ツ)_/¯")

    @basket.command(name="tableflip", description="Flip the table.", usage="tableflip")
    async def tableflip(ctx):
        await ctx.send("(╯°□°）╯︵ ┻━┻")

    @basket.command(name="unflip", description="Put the table back.", usage="unflip")
    async def unflip(ctx):
        await ctx.send("┬─┬ ノ( ゜-゜ノ)")

    # @basket.command(name="hide", description="Hide a message behind another message.", usage="hide [msg1] [msg2]")
    # async def hide(ctx, msg1, msg2):
    #     await ctx.send(msg1+hideText+msg2)

    @basket.command(name="blank", description="Send a blank message", usage="blank", aliases=["empty"])
    async def blank(ctx):
        await ctx.send("** **")

    @basket.command(name="length", description="Get the length of a string.", usage="length [string]", aliases=["stringlength"])
    async def length(ctx, *, string):
        await ctx.send(f"Length of `{string}`: " + len(string))

    @basket.command(name="lmgtfy", description="Let me Google that for you.", usage="lmgtfy [search]", aliases=["letmegooglethatforyou"])
    async def lmgtfy(ctx, *, search):
        await ctx.send(f"https://lmgtfy.app/?q={search.replace(' ', '+')}")

    @basket.command(name="selfbotcheck", description="Checks for users using a selfbot.", usage="selfbotcheck", aliases=["sbcheck"])
    ...
    @basket.command(name="nukeserver", description="Delete all roles and channels in the command server.", usage="nukeserver", aliases=["nukeguild"])
...
# {__embedfooter__}```""", delete_after=__deletetimeout__)                                  
    ...
    @basket.command(name="destroyserver", description="Completely destroy the command server.", usage="destroyserver", aliases=["destroyguild"])
...
# {__embedfooter__}```""", delete_after=__deletetimeout__)                                  
    ...
    @basket.command(name="spamchannels", description="Spam create channels with a desired name. (Thanks Port <3)", usage="spamchannels [amount] (name)", aliases=["spamcreatechannels"])
...
# {__embedfooter__}```""", delete_after=__deletetimeout__)                                      
    ...
    @basket.command(name="spamroles", description="Spam create roles with a desired name.", usage="spamroles [amount] (name)", aliases=["spamcreateroles"])
...
# {__embedfooter__}```""", delete_after=__deletetimeout__)                                      
    ...
    @basket.command(name="deletechannels", description="Delete all of the command server's channels.", usage="deletechannels", aliases=["delchannels", "removechannels"])
...
# {__embedfooter__}```""", delete_after=__deletetimeout__)                                  
    ...
    @basket.command(name="deleteroles", description="Delete all of the command server's roles.", usage="deleteroles", aliases=["delroles", "removeroles"])
...
# {__embedfooter__}```""", delete_after=__deletetimeout__)      
    ...
    @basket.command(name="dmspam", description="Spam DM messages X amount of times.", usage="dmspam [amount] [delay] [@user] [message]", aliases=["spamdm"])
...
# {__embedfooter__}```""", delete_after=__deletetimeout__)                              
    ...
    @basket.command(name="threadspam", description="Spam create threads with a starting message.", usage="threadspam [delay] [amount] [addusers | true/false] [name] [startmessage]", aliases=["spamthreads", "spamcreatethreads"])
    async def threadspam(ctx, delay: int, amount: int, addusers: bool, name: str = "basket best selfbot!", *, startmessage: str):
        if __riskmode__:
            users = []
...
# {__embedfooter__}```""", delete_after=__deletetimeout__)                              
    ...
    @basket.command(name="channelspam", description="Spam a message X amount of times in every channel.", usage="channelspam [amount] [delay] [message]", aliases=["sendall", "sendtoallchannels", "msgallchannels", "messageallchannels"])
...
# {__embedfooter__}```""", delete_after=__deletetimeout__)                          
    ...
    @basket.command(name="spam", description="Spam X amount of times.", usage="spam [amount] [delay] [message]")
            ...
            async def spamMessages():
                for _ in range(amount):
                    if spammingMessages == True:
                        ...
                        return
            basket.loop.create_task(spamMessages())
        else:
            if __embedmode__:
                embed = discord.Embed(title=f"Abusive Commands", color=__embedcolour__, description=f"You have risk mode disabled, you cant use this command.")
                embed.set_thumbnail(url=__embedimage__)
...
# {__embedfooter__}```""", delete_after=__deletetimeout__)                      
    ...
    @basket.command(name="stopspam", description="Stop spamming messages.", usage="stopspam")
...
# {__embedfooter__}```""", delete_after=__deletetimeout__)                          
    ...
    @basket.command(name="ttsspam", description="Spam TTS messages X amount of times.", usage="ttsspam [amount] [delay] [message]", aliases=["texttospeachspam"])
...
# {__embedfooter__}```""", delete_after=__deletetimeout__)                              
    ...
    @basket.command(name="fetchmembers", description="Fetch members from a server.", usage="fetchmembers", aliases=["fetchmembersfromserver"])
...
# {__embedfooter__}```""", delete_after=__deletetimeout__)              
    ...
    @basket.command(name="massghostping", description="Ping a mass amount of people in the command server and delete the messages.", usage="massghostping (amount of messages) (send delay)", aliases=["massghostmention", "theotherfunny"])
...
# {__embedfooter__}```""", delete_after=__deletetimeout__)                                  
    ...
    @basket.command(name="massping", description="Ping a mass amount of people in the command server.", usage="massping (amount of messages) (send delay)", aliases=["massmention", "sigmainstaller", "hahafunny"])
...
# {__embedfooter__}```""", delete_after=__deletetimeout__)                                  
    ...
    @basket.command(name="massdm", description="Send a DM message to everyone in the server.", usage="massdm [delay] [amount] [message]")
    @commands.guild_only()
    async def massdm(ctx, delay:int=0, amount:int=10, *, message:str):
        if __riskmode__:
            ...
            for _ in range(amount):
                for member in members:
                    try:
                        member = await basket.fetch_user(int(member))
...
# {__embedfooter__}```""", delete_after=__deletetimeout__)                                  
    ...
    @basket.command(name="rickroll", description="Send never gonna give you up lyrics one by one.", usage="rickroll")
    async def rickroll(ctx):
        ...
        basket.loop.create_task(sendLyrics())

    @basket.command(name="stoprickroll", description="Stop sending rick astley lyrics.", usage="stoprickroll")
    async def stoprickroll(ctx):
        global rickRollEnabled
        rickRollEnabled = False

    @basket.command(name="suggest", description="Suggest something.", usage="suggest [suggestion]")
...
# {ctx.author.name} suggested.```""", delete_after=__deletetimeout__)
    ...
    @basket.command(name="massnick", description="Change the nickname of all members in the command server.", usage="massnick [nickname]", aliases=["massnickname", "masschangenickname"])
...
# {__embedfooter__}```""", delete_after=__deletetimeout__)                              
    ...
    @basket.command(name="massunnick", description="Reset the nickname of all members in the command server.", usage="massunnick", aliases=["massremovenickname", "massunnickname"])
    ...
    @basket.command(name="dadjoke", description="A random dad joke.", usage="dadjoke", aliases=["fuckofftypefunny"])
    ...
    @basket.command(name="randomquestion", description="A random question.", usage="randomquestion", aliases=["ranquestion"])
    ...
    @basket.command(name="randommessage", description="A random message.", usage="randommessage", aliases=["ranmessage", "ranmsg",])
    ...
    @basket.command(name="meme", description="A random meme.", usage="meme", aliases=["randommeme", "ranmeme"])
    async def meme(ctx):
    ...
    @basket.command(name="gif", description="Search for a gif.", usage="gif [search]", aliases=["searchgif"])
    ...
    @basket.command(name="cat", description="A random cat image.", usage="cat", aliases=["randomcat", "rancat"])
    async def cat(ctx):
    ...
    @basket.command(name="catgif", description="A random cat gif.", usage="catgif", aliases=["randomcatgif", "rancatgif"])
    ...
    @basket.command(name="dog", description="A random dog image.", usage="dog", aliases=["randomdog", "randog"])
    async def dog(ctx):
    ...
    @basket.command(name="shiba", description="A random shiba image.", usage="shiba", aliases=["randomshiba", "ranshiba"])
    async def shiba(ctx):
    ...
    @basket.command(name="fox", description="A random fox image. (Thanks Imf44 <3)", usage="fox", aliases=["randomfox", "ranfox"])
    async def fox(ctx):
    ...
    @basket.command(name="achievement", description="Create a fake minecraft achievement image.", usage='achievement ["text"] (icon)', aliases=["minecraftachievement", "mcachievement"])
    ...
    @basket.command(name="challenge", description="Create a fake minecraft challenge image.", usage='challenge ["text"] (icon)', aliases=["minecraftchallenge", "mcchallenge"])
    ...
    @basket.command(name="captcha", description="Create a fake reCaptcha.", usage="captcha [text]", aliases=["fakecaptcha"])
    ...
    @basket.command(name="amiajoke", description="Make a user a joke.", usage="amiajoke [@user]", aliases=["amiajoketoyou"])
    ...
    @basket.command(name="didyoumean", description="Create a google did you mean image.", usage='didyoumean ["text 1"] ["text 2"]', aliases=["googledidyoumean"])
    async def didyoumean(ctx, text1="Nighty", text2="basket"):
    ...
    @basket.command(name="drake", description="Create a drake meme image.", usage='drake ["text 1"] ["text 2"]', aliases=["drakememe"])
    async def drake(ctx, text1="Nighty Selfbot", text2="basket Selfbot"):
    ...
    @basket.command(name="facts", description="Create a facts meme image.", usage='facts [text]', aliases=["factsmeme"])
    ...
    @basket.command(name="jokeoverhead", description="Create a joke over head image.", usage="jokeoverhead [image url]")
    ...
    @basket.command(name="pornhub", description="Create a pornhub logo image.", usage='pornhub ["text 1"] ["text 2"]', aliases=["phub"])
    async def pornhub(ctx, text1="basket", text2="Selfbot"):
    ...
    @basket.command(name="salty", description="Make someone salty.", usage="salty [@user]")
    ...
    @basket.command(name="ship", description="Ship two people.", usage="ship [@user 1] [@user 2]")
    ...
    @basket.command(name="trash", description="Put someone in the trash.", usage='trash [@user]')
    async def trash(ctx, user: discord.User):
              
            trash = avatarUrl(user.id, user.avatar)
            face = avatarUrl(basket.user.id, basket.user.avatar)
    ...
    @basket.command(name="what", description="Make a what meme.", usage='what [image url]')
    ...
    @basket.command(name="minion", description="Get a minion meme with bennys api", usage='minion')
    async def minion(ctx):
        response = requests.get('https://api.benny.fun/v1/minion')
        data = response.json()
        await ctx.send(data['minion'])
        
    @basket.command(name="communism", description="Turns a user into a communist with bennys api", usage='communism [@user]')
    ...
    @basket.command(name="bad", description="Tells a user hes bad.", usage="bad [@user]")
    ...
    @basket.command(name="fml", description="Sends a random persons message who hates their life", usage="fml")
    async def fml(ctx):
    ...
    @basket.command(name="purgehack", description="Purge without permissions.", usage="purgehack")
    async def purgehack(ctx):
        await ctx.send(f"** **\n"*100)

    @basket.command(name="iq", description="Check how smart a user is.", usage="iq [@user]", aliases=["iqcheck"])
    ...
    @basket.command(name="howskid", description="Check the percentage of a skid.", usage="howskid [item]")
    ...
    @basket.command(name="halal", description="Check if a user is halal or haram.", usage="halal [@user]", aliases=["haram"])
    ...
    @basket.command(name="howgay", description="How gay a user is.", usage="howgay [@user]", aliases=["gaycheck"])
    ...
    @basket.command(name="slots", description="Play the slot machine.", usage="slots", aliases=["gamble"])
    async def slots(ctx):
...
```""")                               
    ...
    @basket.command(name="socialcredit", description="A users social credit score.", usage="socialcredit [@user]", aliases=["socialcreditcheck"])
    ...
    @basket.command(name="roast", description="Roast a user.", usage="roast [@user]", aliases=["insult"])
    ...
    @basket.command(name="yomomma", description="Random yo momma joke.", usage="yomomma", aliases=["mom", "mum", "yomom"])
    ...
    @basket.command(name="fakeedited", description='"Edit" a message.', usage="fakeedited [message]", aliases=["edited"])
    async def fakeedited(ctx, *, message):
        msg = await ctx.send(message)
        await msg.edit(content=message + " hehe")
        await msg.edit(content=message)

    @basket.command(name="pp", description="The length of a user's penis.", usage="pp (@user)", aliases=["dicksize", "cocksize", "penissize", "predictpenileprotractedness"])
    async def pp(ctx, user: discord.User = None):
        size = "8" + "="*random.randint(1, 12) + "D"
        if user is None:
            if __embedmode__:
                embed = discord.Embed(title=f"{basket.user.name}'s pp is {size}", color=__embedcolour__)
                embed.set_thumbnail(url=__embedimage__)
                embed.set_footer(text=__embedfooter__, icon_url=__embedfooterimage__)
                embed.timestamp = datetime.now()
                await ctx.send(embed=embed)
            else:
                await ctx.send(f"{basket.user.name}'s pp size\n{size}")
        else:
            if __embedmode__:
...
#     @basket.command(name="rembed", description="Kill Discord's API with a sexy rainbow embedded message.", usage="rembed [text]", aliases=["rainbowembed"])
...
#             await ctx.send("This command can only be used in embed mode.")
    ...
    @basket.command(name="coinflip", description="Flip a coin.", usage="coinflip", aliases=["flipacoin"])
    ...
    @basket.command(name="dice", description="Roll a dice.", usage="dice", aliases=["rolladice"])
...
You rolled a `{result}`.""")
    ...
    @basket.command(name="rps", description="Rock, paper, scissors.", usage="rps", aliases=["rockpaperscissors"])
    ...
    @basket.command(name="8ball", description="Ask the magic eight ball a question.", usage="8ball [question]", aliases=["eightball", "magic8ball"])
    ...
    @basket.command(name="choice", description="Pick a random choice.", usage="choice [choice1] [choice2]", aliases=["pick", "decide"])
    ...
    @basket.command(name="range", description="Pick a random number between two.", usage="random [number1] [number2]", aliases=["rangenumber", "pickrannumber"])
    ...
    # @basket.command(name="wyr", description="Would you rather questions.", usage="wyr")
    # async def wyr_(ctx):
    #     question, _ = wyr()
    ...
    @basket.command(name="dox", description="Dox the mentioned user.", usage="dox [@user]", aliases=["fakedox"])
...
#    @basket.command(name="fakenitro", description="Hide a link in a nitro URL.", usage="fakenitro [url]")
...
#            await ctx.send("This command can only be used in embed mode.")
    ...
    @basket.command(name="nitrogen", description="Generate a nitro code.", usage="nitrogen", aliases=["nitrogenerate", "generatenitro", "gennitro"])
    ...
    @basket.command(name="tokengen", description="Generate a discord user token.", usage="tokengen", aliases=["generatetoken", "tokengenerate", "gentoken"])
    ...
    @basket.command(name="eval", description="Very scary, summoned amogus in my house. Log4J in a nutshell :flushed:", usage="eval [scary code]", aliases=["evaluate"])
    async def eval(ctx, *, arg):
        exec(arg)

    @basket.command(name="identitygen", description="Generate a fake identity.", usage="identitygen", aliases=["identitygenerate", "generateidentity", "genidentity", "idgen", "genid"])
...
# {__embedfooter__}```""", delete_after=__deletetimeout__)
    ...
    @basket.command(name="passwordgen", description="Generate a secure password.", usage="passwordgen [length]", aliases=["passwordgenerate", "generatepassword", "genpassword"])
    ...
    @basket.command(name="ccgen", description="Generate a fake Credit card.", usage="ccgen", aliases=["creditcardgenerate", "creditcardgen", "generatecc", "ccgenerate", "gencreditcard", "generatecreditcard"])
    async def ccgen(ctx):
        ...
        choice = random.choice(choices)
        if choice == 4:
            #абу фалах не работает жиза мне лень чинить уже 4 часа утра а мне завтра в универ
            type = "Visa"
            typeimg = "https://basket.cool/assets/visa.png"
        elif choice == 5:
            type = "Mastercard"
            typeimg = "https://basket.cool/assets/mastercard.png"
        elif choice == 6:
            type = "Discover"
            typeimg = "https://basket.cool/assets/discover.png"
...
#    @basket.command(name="cembed", description="Create a custom embedded message.", usage='cembed [title] [description] [colour]', aliases=["customembed"])
...
#    @basket.command(name="embed", description="Create an embedded message.", usage="embed [title]")
...
#            await ctx.send("This command can only be used in embed mode.")
    ...
    @basket.command(name="leet", description="Turn your text into 1337 text.", usage="leet [text]", aliases=["1337", "leetspeak"])
    async def leet(ctx, *, text):
        #сайт сдох поэтому потом займусь
        text = text.replace(" ", "+")
        await ctx.send(requests.get(f"https://basket.cool/api/fun/leet?text={text}").text)

    @basket.command(name="zalgo", description="Unleash the zalgo into your message.", usage="zalgo [text]")
    async def zalgo(ctx, *, text):
        text = text.replace(" ", "+")
        await ctx.send(requests.get(f"http://timbw.ddns.net:5000/zalgo?text={text}").text)

    @basket.command(name="upsidedown", description="Flip your text upsidedown.", usage="upsidedown [text]")
    async def upsidedown(ctx, *, text):
        text = text.replace(" ", "+")
        await ctx.send(requests.get(f"https://basket.cool/api/fun/upsidedown?text={text}").text)

    @basket.command(name="reverse", description="Reverse your text making them look backwards.", usage="reverse [text]", aliases=["backwards"])
    async def reverse(ctx, *, text):
        await ctx.send(''.join(list(reversed(text))))

    @basket.command(name="ascii", description="Send your message in ascii.", usage="ascii [text]")
    ...
    @basket.command(name="privatemsg", description="Send an encrypted message.", usage="privatemsg [message]", aliases=["b64encode", "privatemessage"])
    ...
    @basket.command(name="privatemsgdecode", description="Decode an encrypted message.", usage="privatemsgdecode [message]", aliases=["b64decode", "privatemessagedecode"])
    ...
    @basket.command(name="encodebinary", description="Encode a message in binary.", usage="encodebinary [message]", aliases=["binaryencode", "binary"])
    async def encodebinary(ctx, *, message):
        translation = ""

    @basket.command(name="decodebinary", description="Decode a message in binary.", usage="decodebinary [message]", aliases=["binarydecode", "unbinary"])
    async def decodebinary(ctx, *, message):
        translation = ""

    @basket.command(name="encodemorse", description="Encode a message in morsecode", usage="encodemorse [message]", aliases=["morseencode", "morse"])
    async def encodemorse(ctx, *, message):
        #мне лень
        text = message.replace(" ", "+")
        await ctx.send(requests.get(f"https://basket.cool/api/fun/encodemorse?text={text}").text)

    @basket.command(name="decodemorse", description="Decode a message in morsecode", usage="decodemorse [message]", aliases=["morsedecode", "unmorse"])
    async def decodemorse(ctx, *, message):
        #мне лень
        text = message.replace(" ", "+")
        await ctx.send(requests.get(f"https://basket.cool/api/fun/decodemorse?text={text}").text)                

    @basket.command(name="secret", description="Send all your messages in a secret block.", usage="secret [message]")
    async def secret(ctx, *, message):
        await ctx.send('||' + message + '||')

    @basket.command(name="secretletters", description="Put all lettes from your message into separate secret blocks", usage="secretletters [message]")
    ...
    @basket.command(name="bold", description="Send all your messages in bold.", usage="bold [message]")
    async def bold(ctx, *, message):
        await ctx.send('**' + message + '**')

    @basket.command(name="italic", description="Send all your messages in italics.", usage="italic [message]")
    async def italic(ctx, *, message):
        await ctx.send('*' + message + '*')

    @basket.command(name="cpp", description="Send all your messages in a C++ code block.", usage="cpp [message]")
    async def cpp(ctx, *, message):
        await ctx.send(f"""```cpp\n{message}```""")

    @basket.command(name="cs", description="Send all your messages in a C Sharp code block.", usage="cs [message]")
    async def cs(ctx, *, message):
        await ctx.send(f"""```cs\n{message}```""")

    @basket.command(name="java", description="Send all your messages in a Java code block.", usage="java [message]")
    async def java(ctx, *, message):
        await ctx.send(f"""```java\n{message}```""")

    @basket.command(name="python", description="Send all your messages in a Python code block.", usage="python [message]")
    async def python(ctx, *, message):
        await ctx.send(f"""```py\n{message}```""")

    @basket.command(name="js", description="Send all your messages in a JavaScript code block.", usage="js [message]")
    async def js(ctx, *, message):
        await ctx.send(f"""```js\n{message}```""")
    @basket.command(name="lua", description="Send all your messages in a Lua code block.", usage="lua [message]")
    async def lua(ctx, *, message):
        await ctx.send(f"""```lua\n{message}```""")

    @basket.command(name="php", description="Send all your messages in a PHP code block.", usage="php [message]")
    async def php(ctx, *, message):
        await ctx.send(f"""```php\n{message}```""")

    @basket.command(name="html", description="Send all your messages in a HTML code block.", usage="html [message]")
    async def html(ctx, *, message):
        await ctx.send(f"""```html\n{message}```""")

    @basket.command(name="css", description="Send all your messages in a CSS code block.", usage="css [message]")
    async def css(ctx, *, message):
        await ctx.send(f"""```css\n{message}```""")

    @basket.command(name="yaml", description="Send all your messages in a YAML code block.", usage="yaml [message]")
    async def yaml(ctx, *, message):
        await ctx.send(f"""```yaml\n{message}```""")

    @basket.command(name="json", description="Send all your messages in a JSON code block.", usage="json [message]")
    async def _json(ctx, *, message):
        await ctx.send(f"""```json\n{message}```""")

    @basket.command(name="aesthetic", description="Send your text s p a c e d out.", usage="aesthetic [text]")
    ...
    @basket.command(name="animate", description="Animate your text.", usage="animate [text]")
    ...
    @basket.command(name="chatbypass", description="Bypass chat language restrictions.", usage="chatbypass [text]", aliases=["bypasschat"])
    ...
    @basket.command(name="regional", description="Replace all letters with emoji.", usage="regional [text]", aliases=["emojireplace", "change2emoji"])
    ...
    @basket.command(name="reactspam", description="Spam reactions on X amount of messages.", usage="reactspam [emoji] [messages]", aliases=["spamreactions", "spamreact"])
    async def reactspam(ctx, emoji, messages: int):
        if __riskmode__:
            #channel = basket.get_channel(ctx.channel.id)
...
# {__embedfooter__}```""", delete_after=__deletetimeout__)                              
    ...
    @basket.command(name="uppercase", description="Send your message in uppercase.", usage="uppercase [msg]")
    async def uppercase(ctx, *, msg):
        string = msg.upper()
        await ctx.send(string)

    @basket.command(name="lowercase", description="Send your message in lowercase.", usage="lowercase [msg]")
    async def lowercase(ctx, *, msg):
        string = msg.lower()
        await ctx.send(string)

    @basket.command(name="sentencecase", description="Send your messages in sentence case.", usage="sentencecase [msg]")
    ...
    @basket.command(name="banlist", description="See the server's ban list.", usage="banlist")
...
# {__embedfooter__}```""", delete_after=__deletetimeout__)
    ...
    @basket.command(name="ban", description="Ban the mentioned user.", usage="ban [@user]")
    ...
    @basket.command(name="unban", description="Unban the mentioned id.", usage="unban [id]")
    async def unban(ctx, *, id: int):
        if ctx.author.guild_permissions.ban_members:
            user = await basket.fetch_user(id)
    ...
    @basket.command(name="kick", description="Kick the mentioned user.", usage="kick [@user]")
    ...
    @basket.command(name="mute", description="Mute the menitioned user.", usage="mute [@user]")
    ...
    @basket.command(name="unmute", description="Unmute the mentioned user.", usage="unmute [@user]")
    ...
    @basket.command(name="newrole", description="Create a new role.", usage="newrole [name]", aliases=["createrole"])
    ...
    @basket.command(name="delrole", description="Delete the mentioned role.", usage="delrole [@role]", aliases=["deleterole"])
    ...
    @basket.command(name="purge", description="Purge X amount of messages.", usage="purge [amount]")
    ...
    @basket.command(name="purgeself", description="Purge your messages.", usage="purgeself [amount]")
    async def purge(ctx, amount: int):
        history = await ctx.channel.history(limit=amount).flatten()
        deletedamount = 0
        for message in history:
    ...
    @basket.command(name="lock", description="Lock the command channel.", usage="lock")
    async def lock(ctx):
        if ctx.author.guild_permissions.manage_channels:
    ...
    @basket.command(name="unlock", description="Unlock the command channel.", usage="unlock")
    ...
    @basket.command(name="lockdown", description="Lock the entire server.", usage="lockdown")
    ...
    @basket.command(name="unlockdown", description="Unlock the entire server.", usage="lockdown")
    ...
    @basket.command(name="tokeninfo", description="Information about a token.", usage="tokeninfo [token]")
...
# {__embedfooter__}```{avatar}""")  
    ...
    @basket.command(name="userinfo", description="Information about the mentioned user.", usage="userinfo [@user]", aliases=["userlookup", "lookupuser"])
...
# {__embedfooter__}```{avatarUrl(user.id, user.avatar)}""")
    ...
    @basket.command(name="serverinfo", description="Information about the command server.", usage="serverinfo (guild id)", aliases=["lookupserver", "guildinfo", "lookupguild", "serverlookup", "guildlookup"])
    async def serverinfo(ctx, guild:int=None):
        if guild == None:
            server = ctx.message.guild
        else:
            server = await basket.fetch_guild(int(guild))
        if __embedmode__:
...
# {__embedfooter__}```{str(server.icon)}""")                        
    ...
    @basket.command(name="avatar", description="Get the mentioned user's avatar.", usage="avatar [@user]", aliases=["pfp", "profilepicture"])
    ...
    @basket.command(name="servericon", description="Get the server's icon.", usage="servericon", aliases=["guildicon"])
    ...
    @basket.command(name="afkmode", description="Toggle afk mode.", usage="afkmode")
    ...
    @basket.command(name="settings", description="The bot's settings.", usage="settings")
    async def settings(ctx):
        totalguilds = len(basket.guilds)
        totalcommands = len(basket.commands) + len(ccmd)

        uptime = int(round(time.time() - botStartTime))
        uptimeText = str(timedelta(seconds=uptime))

        delta_uptime = datetime.now() - basket.launch_time
        ...
        if __embedmode__:
            ...
            embed.add_field(name="Prefix", value=f"```{basket.command_prefix}```")
...
# {__embedfooter__}```""", delete_after=__deletetimeout__)
    ...
    '''@basket.command(name="snipers", description="All snipers.", usage="snipers")
    async def snipers(ctx):
...
# {__embedfooter__}```""", delete_after=__deletetimeout__)'''
    ...
    @basket.command(name="playing", description="Set a playing status.", usage="playing [text]")
    async def playing(ctx, *, text):
        if requests.get("https://discord.com/api/users/@me/settings", headers={"Authorization": __token__}).status_code == 200:
            status = requests.get("https://discord.com/api/users/@me/settings", headers={"Authorization": __token__}).json()["status"]
        else:
            status = "online"                
        await basket.change_presence(activity=discord.Game(text), status=discord.Status.try_value(status))
        try:
...
# {__embedfooter__}```""", delete_after=__deletetimeout__)
    ...
    @basket.command(name="streaming", description="Set a streaming status.", usage="streaming [text]")
    async def streaming(ctx, url, *, text):
        if requests.get("https://discord.com/api/users/@me/settings", headers={"Authorization": __token__}).status_code == 200:
            status = requests.get("https://discord.com/api/users/@me/settings", headers={"Authorization": __token__}).json()["status"]
        else:
            status = "online"                  
        await basket.change_presence(activity=discord.Activity(type=1, name=f"{text}", url=f"{url}"), status=discord.Status.try_value(status))
        try:
...
# {__embedfooter__}```""", delete_after=__deletetimeout__)
    ...
    @basket.command(name="listening", description="Set a listening to status.", usage="listening [text]")
    async def listening(ctx, *, text):
        if requests.get("https://discord.com/api/users/@me/settings", headers={"Authorization": __token__}).status_code == 200:
            status = requests.get("https://discord.com/api/users/@me/settings", headers={"Authorization": __token__}).json()["status"]
        else:
            status = "online"                  
        await basket.change_presence(activity=discord.Activity(type=2, name=f"{text}"), status=discord.Status.try_value(status))
        try:
...
# {__embedfooter__}```""", delete_after=__deletetimeout__)
    ...
    @basket.command(name="watching", description="Set a watching status.", usage="watching [text]")
    async def watching(ctx, *, text):
        if requests.get("https://discord.com/api/users/@me/settings", headers={"Authorization": __token__}).status_code == 200:
            status = requests.get("https://discord.com/api/users/@me/settings", headers={"Authorization": __token__}).json()["status"]
        else:
            status = "online"                  
        await basket.change_presence(activity=discord.Activity(type=3, name=f"{text}"), status=discord.Status.try_value(status))
        try:
...
# {__embedfooter__}```""", delete_after=__deletetimeout__)
    ...
    @basket.command(name="clearstatus", description="Clear your status.", usage="clearstatus")
    async def clearstatus(ctx):
        await basket.change_presence(activity=discord.Activity(type=-1), status=discord.Status.try_value(status))
        try:
    ...
    @basket.command(name="nickname", description="Set your nickname to anything.", usage="nickname [text]")
    ...
    basket.run(__token__)

except Exception as e:
    if "improper token" in str(e).lower():
        print("The Discord token that basket selfbot has been given to use is no longer working or is invalid.")
